...
#	print("carregado id: %d" % id, Infospots)
	...
	#(angle:Vector3, hotspot_name:String, id:int, vrsc, InfoSpot:bool = false)
	...
	current_location = id
	targeted_location = -1
	if new_adjacents.size() > 0: $Supa.GetEnvironmentRows(new_adjacents)
	ReduceSteps()
